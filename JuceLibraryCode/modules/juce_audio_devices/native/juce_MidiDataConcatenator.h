        ...
        do
        {
            if (pendingBytes > 0 && *d >= 0x80)
            {
                ...
                else
                {
					...
					// BEGIN_MODIFIED_BY_RESOLUME // added else statement: data bytes with bit 7 are malformed, quit parsing
					else
					{
						Logger::writeToLog("Parsing databyte with bit 7 set, this is illegal, stopping parsing of sysex..");

...
void MidiOutput::sendMessageNow (const MidiMessage& message)
{
    ...
    else
    {
		...
		int result = midiOutShortMsg (handle->handle, *(unsigned int*) message.getRawData());
        #ifdef JUCE_DEBUG
		switch(result)
		{
			...
			case MIDIERR_NOTREADY:
                ...
                // this is code from current juce tip, it seems like a 50 times retry loop
                for (int i = 0; i < 50; ++i)
                {
			...
			case MMSYSERR_NOERROR:
				//Sleep(2);
				break;
#pragma MODIFIED_BY_RESOLUME // added these

...
public:
	...
	ComponentPeer* getComponentPeer()				{ return nativeWindow; }
...
private:
    ...
    {
		//BEGIN_MODIFIED_BY_RESOLUME
		// current context must be NULL to properly delete
		// gDebugger still does not mark contexts as deleted in list, but log says it has been deleted....
		if( renderContext != 0 )
		{
			if( wglGetCurrentContext() != 0 )
				if( !wglMakeCurrent( dc, 0 ) )
				{
					Logger::outputDebugString( String( "Cant make current to 0" ) );
					jassertfalse;
				}

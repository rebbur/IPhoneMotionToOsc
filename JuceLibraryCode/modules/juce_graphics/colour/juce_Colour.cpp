...
static Array<uint32> colourTable;
static Array<uint32> colourReplace;

void Colour::addColourToReplace( uint32 from, uint32 to )
{
    colourTable.add(from);
    colourReplace.add(to);
}

void Colour::checkColourForReplacement()
{
	//((alpha << 24) | (red << 16) | (green << 8) | blue)
    int index = colourTable.indexOf(argb.getInARGBMaskOrder());
    if (index != -1)
	{
		uint32 colourtoReplace = colourReplace.getUnchecked(index);
        argb = PixelARGB((colourtoReplace >> 24) & 0xff,
                         (colourtoReplace >> 16) & 0xff,
                         (colourtoReplace >> 8) & 0xff,
                         (colourtoReplace >> 0) & 0xff);
        
//        argb = PixelARGB((colourtoReplace >> 8) & 0xff,
//						 (colourtoReplace >> 16) & 0xff,
//						 (colourtoReplace >> 8) & 0xff,
//						 (colourtoReplace >> 0) & 0xff);

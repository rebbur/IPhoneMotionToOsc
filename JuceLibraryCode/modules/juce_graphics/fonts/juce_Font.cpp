...
static float BaseFontHeight = 0.0f;
...
public:
    ...
    SharedFontInternal (int styleFlags, float fontHeight) noexcept
        : typefaceName (Font::getDefaultSansSerifFontName()),
          ...
          // BEGIN_MODIFIED_BY_RESOLUME
          height (fontHeight+BaseFontHeight),
          // END_MODIFIED_BY_RESOLUME
    ...
    SharedFontInternal (const String& name, const String& style, float fontHeight) noexcept
        : typefaceName (name), typefaceStyle (style),
          // BEGIN_MODIFIED_BY_RESOLUME
          height (fontHeight+BaseFontHeight),
          // END_MODIFIED_BY_RESOLUME
          horizontalScale (1.0f), kerning (0), ascent (0), underline (false)
    ...
    explicit SharedFontInternal (const Typeface::Ptr& face) noexcept
        : typeface (face),
          ...
          // BEGIN_MODIFIED_BY_RESOLUME
          height (FontValues::defaultFontHeight+BaseFontHeight),
          // END_MODIFIED_BY_RESOLUME
          horizontalScale (1.0f), kerning (0), ascent (0), underline (false)
    {
...
// BEGIN_MODIFIED_BY_RESOLUME
void Font::setBaseFontHeight(float height)
{
	BaseFontHeight = height;
}
// BEGIN_MODIFIED_BY_RESOLUME
...
{
    // BEGIN_MODIFIED_BY_RESOLUME
    newHeight = FontValues::limitFontHeight (newHeight+BaseFontHeight);
    // END_MODIFIED_BY_RESOLUME
...
void Font::findFonts (Array<Font>& destArray)
{
    ...
    for (int i = 0; i < names.size(); ++i)
    {
        ...
        if (! styles.contains (style, true))
            style = styles[0];
        // BEGIN_MODIFIED_BY_RESOLUME
        destArray.add (Font (names[i],  style, FontValues::defaultFontHeight+BaseFontHeight));
        // END_MODIFIED_BY_RESOLUME
    }
}
...
Font Font::fromString (const String& fontDescription)
{
    ...
    // BEGIN_MODIFIED_BY_RESOLUME
    return Font (name, style, height+BaseFontHeight);
    // END_MODIFIED_BY_RESOLUME
}

...
Font LookAndFeel::getNumericLabelFont (NumericLabel& label)
{
    return label.getFont();
}

void LookAndFeel::drawNumericLabel (Graphics& g, NumericLabel& label)
{
	g.fillAll (label.findColour (NumericLabel::backgroundColourId));
	
	if (! label.isBeingEdited())
	{
		const float alpha = label.isEnabled() ? 1.0f : 0.5f;
		const Font font (getNumericLabelFont (label));
		
		g.setColour (label.findColour (NumericLabel::textColourId).withMultipliedAlpha (alpha));
		g.setFont (font);
		...
		g.setColour (label.findColour (NumericLabel::outlineColourId).withMultipliedAlpha (alpha));
	}
	else if (label.isEnabled())
	{
		g.setColour (label.findColour (NumericLabel::outlineColourId));
	}

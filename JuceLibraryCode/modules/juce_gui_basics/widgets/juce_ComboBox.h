...
public:
    ...
    */
    ...
    //BEGIN_MODIFIED_BY_RESOLUME
		buttonOverColourId  = 0x1000f00
    //END_MODIFIED_BY_RESOLUME
    };
	
	
	//BEGIN_MODIFIED_BY_RESOLUME
    /** This can be used to set a custom buttonArea in to which to draw button for the combobox
	as a result different button coordinates are give to the drawCombobox call
	*/
	void setButtonArea(const Rectangle<int>& area) noexcept;

	void setRoundedCorners(bool roundedCorners) noexcept;
	bool hasRoundedCorners() const noexcept;
	//END_MODIFIED_BY_RESOLUME
...
protected:
	...
	ItemInfo* getItemForIndex (int) const noexcept; // made protected
	String textWhenNothingSelected, noChoicesMessage; // made protected
...
private:
	...
	*/
    ...
    //String textWhenNothingSelected, noChoicesMessage; // MODIFIED_BY_RESOLUME: made protected
    EditableState labelEditableState;

    ItemInfo* getItemForId (int) const noexcept;
	
    //ItemInfo* getItemForIndex (int) const noexcept; // MODIFIED_BY_RESOLUME: made protected

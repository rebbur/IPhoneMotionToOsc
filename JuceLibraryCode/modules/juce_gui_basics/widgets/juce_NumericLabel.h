...
#ifndef JUCE_NUMERICLABEL_H_INCLUDED
#define JUCE_NUMERICLABEL_H_INCLUDED
...
/**
    ...
    editor when clicked.
*/
class JUCE_API  NumericLabel  : public Component,
...
public:
    //==============================================================================
    /** Creates a NumericLabel.

        @param componentName    the name to give the component
        @param labelText        the text to show in the label
    */
    NumericLabel (const String& componentName = String(),
    ...
    /** Changes the label text.
        ...
        any NumericLabel::Listener objects if the new text is different.
    */
    void setText (const String& newText,
                  NotificationType notification);

	//BEGIN_MODIFIED_BY_RESOLUME
	void setFullText(const String& newText);
    ...
    */
    ...
    //==============================================================================
    /**
        A class for receiving events from a NumericLabel.

        You can register a NumericLabel::Listener with a NumericLabel using the NumericLabel::addListener()
        method, and it will be called when the text of the label changes, either because
        of a call to NumericLabel::setText() or by the user editing the text (if the label is
        editable).

        @see NumericLabel::addListener, NumericLabel::removeListener
    */
    ...
    public:
        ...
        /** Called when a NumericLabel's text has changed. */
        virtual void labelTextChanged (NumericLabel* labelThatHasChanged) = 0;

        /** Called when a NumericLabel goes into editing mode and displays a TextEditor. */
        virtual void editorShown (NumericLabel*, TextEditor&) {}

        /** Called when a NumericLabel is about to delete its TextEditor and exit editing mode. */
        virtual void editorHidden (NumericLabel*, TextEditor&) {}
    };
    ...
    */
    struct JUCE_API  LookAndFeelMethods
    {
        ...
        virtual void drawNumericLabel (Graphics&, NumericLabel&) = 0;
        virtual Font getNumericLabelFont (NumericLabel&) = 0;
    };
...
private:
    ...
    String stripTrailingZeros(const String& text);
    bool updateFromTextEditorContents (TextEditor&);

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (NumericLabel)
};

/** This typedef is just for compatibility with old code - newer code should use the NumericLabel::Listener class directly. */
typedef NumericLabel::Listener NumericLabelListener;

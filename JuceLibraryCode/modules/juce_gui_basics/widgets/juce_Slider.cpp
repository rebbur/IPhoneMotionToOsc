...
// made a modification to the contructor fields, swapped LabelListener to NumericLabelListener
// END_MODIFIED_BY_RESOLUME

class Slider::Pimpl   : public AsyncUpdater,
                        public ButtonListener,  // (can't use Button::Listener due to idiotic VC2005 bug)
                        //public LabelListener,
						public NumericLabelListener,
...
public:
    Pimpl (Slider& s, SliderStyle sliderStyle, TextEntryBoxPosition textBoxPosition)
      : owner (s),
		...
		// BEGIN_MODIFIED_BY_RESOLUME
		intervalConstrains(true),		
		displayTrailingZeroes(true),
		numDisplayDecimalPlaces(3),
		textBoxOffsetX(0),
		textBoxOffsetY(0),
		// END_MODIFIED_BY_RESOLUME
        numDecimalPlaces (7),
		...
		wrapRange(false),
		previousMouseDownWasRightMouse(false)
    {
	...
	// BEGIN_MODIFIED_BY_RESOLUME
	void setIntervalConstrains(bool intervalConstrains_)
	{
		intervalConstrains = intervalConstrains_;
	}	
	// END_MODIFIED_BY_RESOLUME
    ...
    void buttonClicked (Button* button) override
    {
		...
		if (mods.isRightButtonDown() || previousMouseDownWasRightMouse) //don't set value when button is clicked with right mouse button
			return;
		// END_MODIFIED_BY_RESOLUME
...
// BEGIN_MODIFIED_BY_RESOLUME
			double delta = 0;
			if(!owner.getButtonIncrement(delta, button == incButton))
				delta = (button == incButton) ? interval : -interval;
// END_MODIFIED_BY_RESOLUME
...
// BEGIN_MODIFIED_BY_RESOLUME
			...
			if(wrapRange)
			{
				if(value > owner.getMaximum())
					value -= (owner.getMaximum() - owner.getMinimum());
				else if(value < owner.getMinimum())
					value += (owner.getMaximum() - owner.getMinimum());
			}
...
// END_MODIFIED_BY_RESOLUME
    ...
    void labelTextChanged (NumericLabel* label) override
    {
		const double newValue = owner.snapValue (owner.getValueFromText (label->getFullText()), notDragging);

        if (newValue != (double) currentValue.getValue())
        {
			// BEGIN_MODIFIED_BY_RESOLUME
			/* (to avoid popping up the PropertyValueEditorComponent)
            DragInProgress drag (*this);
            */
    ...
    void updateText()
    {
        if (valueBox != nullptr)
        {
			...
			newValue = owner.getFullTextFromValue(currentValue.getValue());
			
			if(newValue != valueBox->getFullText())
				valueBox->setFullText(newValue);
        }
    }
    ...
    {
		// BEGIN_MODIFIED_BY_RESOLUME
        if (interval > 0 && intervalConstrains)
    ...
    {
		// BEGIN_MODIFIED_BY_RESOLUME
		mouseDownCustom(e, getThumbIndexAt(e)); // basically the same function, although this function gets the thumb being dragged for us here
		// END_MODIFIED_BY_RESOLuME
    }

    // BEGIN_MODIFIED_BY_RESOLUME
	void mouseDownCustom(const MouseEvent& e, int sliderBeingDragged_)
    {
		// BEGIN_MODIFIED_BY_RESOLUME
		previousMouseDownWasRightMouse = e.mods.isRightButtonDown();
		// END_MODIFIED_BY_RESOLUME
        ...
        if (owner.isEnabled())
        {
            ...
            else if (maximum > minimum)
            {
                ...
                sliderBeingDragged = sliderBeingDragged_; // MODIFIED_BY_RESOLUME original: getThumbIndexAt (e);
				
                minMaxDiff = (double) valueMax.getValue() - (double) valueMin.getValue();
				
                lastAngle = rotaryParams.startAngleRadians + (rotaryParams.endAngleRadians - rotaryParams.startAngleRadians)
    ...
    String textSuffix;
	// BEGIN_MODIFIED_BY_RESOLUME
	bool intervalConstrains, displayTrailingZeroes;
    int numDisplayDecimalPlaces, numDecimalPlaces;
	int textBoxOffsetX, textBoxOffsetY;
	// END_MODIFIED_BY_RESOLUME
    ...
    ScopedPointer<NumericLabel> valueBox;
...
// BEGIN_MODIFIED_BY_RESOLUME
   bool wrapRange;
   bool previousMouseDownWasRightMouse;
// END_MODIFIED_BY_RESOLUME
...
void Slider::setIntervalConstrains(bool intervalConstrains_)
{
	pimpl->intervalConstrains = intervalConstrains_;
}

void Slider::mouseDownCustom(const MouseEvent& e, int sliderBeingDragged_)
{
	pimpl->mouseDownCustom(e, sliderBeingDragged_);
}
...
// BEGIN_MODIFIED_BY_RESOLUME
void Slider::setNumDisplayDecimalPlaces(int newValue)
{
	pimpl->numDisplayDecimalPlaces = newValue;
	pimpl->updateText();
}

void Slider::setDisplayTrailingZeroes(bool _displayTrailingZeroes)
{
	pimpl->displayTrailingZeroes = _displayTrailingZeroes;
	pimpl->updateText();
}

void Slider::setTextBoxOffset(const int x, const int y)
{
	pimpl->textBoxOffsetX = x;
	pimpl->textBoxOffsetY = y;
}
...
String Slider::getTextFromValue (double v)
{
	if (getNumDecimalPlacesToDisplay() > 0)
	{
		...
		// always display number of decimals as indicated by numDisplayDecimalPlaces
		if(pimpl->displayTrailingZeroes)
			n = pimpl->numDisplayDecimalPlaces;
		
		// don't display zeroes, but display decimals
		else 
			n = fmod(v, 1.) == 0. ? 0 : pimpl->numDisplayDecimalPlaces;
		
		return String (v, n) + getTextValueSuffix();
	}
	else
		return String (roundToInt (v)) + getTextValueSuffix();	
}

String Slider::getFullTextFromValue(double v)
{
	if (getNumDecimalPlacesToDisplay())
	{
		// figure out the number of DPs needed to display the value
		int number = v; // get integer part
		double r = v - number; // get remainder

		int numPlaces = 0;
		while(numPlaces < 5 && r != 0.)
		{
			r *= 10; // shift one digit to the left
			r = fmod(r, 1); // get fractional part
			numPlaces++; // increase number of places
		}

		// always display number of decimals as indicated by numDisplayDecimalPlaces
		if(pimpl->displayTrailingZeroes)
		{
			// display at least numDisplayDecimalPlaces
			numPlaces = numPlaces < pimpl->numDisplayDecimalPlaces ? pimpl->numDisplayDecimalPlaces : numPlaces;
		}

		// don't display zeroes, but display decimals
		else 
			numPlaces = fmod(v, 1.) == 0. ? 0 : numPlaces;

		return String (v, numPlaces) + getTextValueSuffix();
	}
	else
		return String (roundToInt (v)) + getTextValueSuffix();
}

void Slider::setWrapsRange(bool wraps)
{
	pimpl->wrapRange = wraps;
}

bool Slider::getWrapsRange() const
{
	return pimpl->wrapRange;
}
...
// BEGIN_MODIFIED_BY_RESOLUME
float Slider::getLinearSliderPos(const double value) { return pimpl->getLinearSliderPos(value); };
// END_MODIFIED_BY_RESOLUME

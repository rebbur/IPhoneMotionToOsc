...
struct ItemComponent  : public Component
{
    ...
    void setHighlighted (bool shouldBeHighlighted)
    {
        ...
        if (isHighlighted != shouldBeHighlighted)
        {
			...
			if (isHighlighted && item.customCallback != nullptr)
				item.customCallback->menuItemHighlighted();
...
bool JUCE_CALLTYPE PopupMenu::isCurrentlyAMenuActive()
{
	return HelperClasses::MenuWindow::getActiveWindows().size() > 0;
}

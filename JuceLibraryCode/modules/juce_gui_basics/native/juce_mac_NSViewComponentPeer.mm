...
private:
	...
	#pragma mark BEGIN_MODIFIED_BY_RESOLUME
    static NSRect constrainFrameRect (id self, SEL, NSRect frameRect, NSScreen* screen)
    {
        if (NSViewComponentPeer* const owner = getOwner (self))
        {
            if (owner->getIgnoreWindowConstrains())
            {
				//don't constrain the rect, we want our window to fully cover the screen
                if (!screen)
                    screen = [self screen];
                return [screen frame];
            }
			...
			{
                return owner->constrainRect (frameRect);
            }
        }
		else
		{
	...
	#pragma end BEGIN_MODIFIED_BY_RESOLUME

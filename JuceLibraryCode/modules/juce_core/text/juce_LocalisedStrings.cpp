...
//#define TRANSLATION_DEBUG 1
#if defined(JUCE_DEBUG) && defined(TRANSLATION_DEBUG)
	String result = translations.getValue (text, "###" + resultIfNotFound);
	if(result.startsWith("###"))
	{
		if(!undefinedTranslations.contains(text))
			undefinedTranslations.add(text);
	}
	// remove text from unused translations array (because this line is obviously used)
	unusedTranslations.remove(StringRef(text));
	return result;
#endif
